...
Starter project Golang + Fiber + PostgreSQL + Zerolog + .env + Migrate (tanpa JWT authentication)
...
cd ..
docker-compose up -d
cd fiber-lite-starter
```

**Option B: PostgreSQL Manual**
```bash
# Buat database
createdb fiber_lite_starter
```
...
- Jika butuh authentication, pertimbangkan gunakan `fiber-jwt-starter`
...
psql -h localhost -U postgres -d fiber_lite_starter
